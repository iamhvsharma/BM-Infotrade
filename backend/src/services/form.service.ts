 ...
 */
export const getPublicForm = async (formId: string): Promise<FormWithFields> => {
  const form = await prisma.form.findFirst({
    where: {
      id: formId,
      isPublished: true,
...
export const submitFormResponse = async (
  ...
  validateFormResponse(form.fields, responseData.data);
...
export const exportFormResponses = async (
  ...
  if (options.format === "csv") {
    return generateCSVExport(form, responses, options.includeMetadata);
  } else {
 ...
 */
const validateFormResponse = (fields: any[], responseData: any): void => {
